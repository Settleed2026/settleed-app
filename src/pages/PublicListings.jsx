...
import { Search, SlidersHorizontal, X, BedDouble, MapPin, ChevronDown, Lock } from 'lucide-react'
...
function AuthGateSheet({ listingId, onClose }) {
  return (
    <>
      {/* Backdrop */}
      <div
      ...
      />
      {/* Bottom sheet */}
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-2xl px-6 pt-5 pb-10 shadow-xl max-w-lg mx-auto">
        <div className="w-10 h-1 bg-gray-200 rounded-full mx-auto mb-5" />
        <div className="flex items-center justify-center w-14 h-14 bg-[#EBF9F4] rounded-full mx-auto mb-4">
        ...
        <p className="text-sm text-gray-500 text-center mb-6 leading-relaxed">
          View contact info, floor plans, and apply online — free for Section 8 tenants.
        </p>
        <Link
          to={`/signup?role=tenant&listing=${listingId}`}
          className="block w-full bg-[#1B3A6B] text-white text-sm font-semibold py-3.5 rounded-xl text-center mb-3"
        >
          Create a Free Account
        </Link>
        <Link
          to={`/login?listing=${listingId}`}
          className="block w-full text-center text-sm text-[#1B3A6B] font-medium py-2"
        >
...
export default function PublicListings() {
  ...
  const [gateListingId, setGateListingId] = useState(null)
  ...
  const fetchListings = useCallback(async () => {
    ...
    if (error) {
      console.error('[PublicListings] Supabase error:', error)
  ...
  function handleCardClick(listingId) {
    if (user && userRole === 'tenant') {
      ...
      // Landlords can browse but redirect to tenant signup
      setGateListingId(listingId)
    } else {
      setGateListingId(listingId)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="bg-[#1B3A6B] px-4 pt-10 pb-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <h1 className="text-white text-lg font-bold">Find Housing</h1>
        </div>
      ...
      {gateListingId && (
        <AuthGateSheet listingId={gateListingId} onClose={() => setGateListingId(null)} />
      )}
    </div>
  )
}
